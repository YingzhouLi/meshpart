...
%
%   A = blockdiags(B,d,m,n) Blockdiags, which generalizes the function
%   "spdiags", produces a sparse matrix with specified block diagonals. A is
%   an m*k-by-n*k matrix, or an m-by-n matrix of k-by-k blocks. The nonzero
%   blocks of A are located on p block diagonals. B is a min(m,n)*k-by-p*k
%   matrix whose k-by-k block columns are the block diagonals of A.
%   (Alternatively, B is k-by-p*k, and then A is block Toeplitz.) d is a
%   vector of p integers in the range -m+1 : n-1, specifying which block
%   diagonals in A are to be nonzero. The values of p and k are determined
%   from the dimensions of B and d.
%
