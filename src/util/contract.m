...
%
%   R = contract(A,blocks) Input: A is an n by n matrix representing a
%   directed or undirected graph. blocks is an n-vector of vertex labels.
%   Output: R is the k-vertex "contracted graph" obtained by contracting
%   each block of A (with like-labeled vertices) into a single node. Edge
%   weights in R are the number of edges joining the blocks, and vertex
%   weights are the number of vertices in the block. If a third input
%   argument "xyA" is present, a second output argument "xyR" is returned,
%   with xy coordinates for the vertices of R: [R,xyR] =
%   contract(A,blocks,xyA)
...
sizes = full(sparse(blocks,1,1,k,1));
