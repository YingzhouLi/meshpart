...
%
%   [gc,gcquality] = sepcircle(A,xyz,ntries) Generate a great circle on the
%   unit sphere in d dimensions, trying to find one that gives a good edge
%   separator of the graph A. "xyz" is the input points, conformally mapped
%   on the unit sphere in d-space so that the approximate centerpoint is at
%   the origin. We return the best of ntries (default 40) attempts. The
%   circle is returned only as a direction, a d-vector normal to its plane.
...
for i = 1 : size(vv,1)
    v = vv(i,:);
    if norm(v) ~= 0
        quality = sepquality(v,A,xyz);
    end
    if gcquality > quality
        gcquality = quality;
        gc = v;
    end
end
