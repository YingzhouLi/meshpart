...
%
%   [center,radius] = mscircle(v,cpt,xyz) This routine, which is only used
%   to draw a picture and for optional return values from geopart, converts
%   the description of the separator in (dim+1)-space to a description of a
%   circle in dim-space in mesh coordinates. v: the normal vector to the
%   separating plane in dim+1 conformal map space. cpt: the centerpoint for
%   the conformal mapping. xyz: the points themselves.
%
%   See GEOPART.
...
circnorms = sum(circ .* circ, 2)';
q = circnorms(1:dim) - ones(1,dim)*circnorms(dim+1);
